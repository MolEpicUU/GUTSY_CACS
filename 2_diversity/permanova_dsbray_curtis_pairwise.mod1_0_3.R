...
#pairwise
dades.fun=dades[dades$casctot_cat==0| dades$casctot_cat==3,]
...
res=data.frame(var1="0",var2="3",R2=fit$R2[1],p.value=fit$"Pr(>F)"[1])

write.table(res,file=paste(output.path,"permanova_adonis2_dsbray_curtis_casctot_pairwise.mod1_0_3.tsv",sep=""),row.names=F,col.names=T,sep="\t")
