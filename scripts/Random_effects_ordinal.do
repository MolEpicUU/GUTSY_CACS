/*Code for running ordinal mixed models with (categorical) CACS as outcome.
*/

global Type "full" /*the way the ordinal CACS is run. alternatives: "full" (with the significant species), "basic" (with all species)*/
...
if "${Type}"=="basic" keep `VList' hg* sis stenosis athero plaque cacstot_cat age carb protein fiber sbp dbp ///
		chol hdl ldl tg bmi sex country smoke diab diab_med pa chol_med ///
		bp_med site_plate* family cacstot_cat
if "${Type}"=="full" keep `VList' sis stenosis athero plaque cacstot_cat age carb protein fiber sbp dbp ///
		chol hdl ldl tg bmi sex country smoke diab diab_med pa chol_med ///
		bp_med site_plate* family cacstot_cat
...
foreach v2 of varlist cacstot_cat{
	cap postclose myfile
	postfile myfile str20 Variable est_mixed pval_mixed lci_mixed hci_mixed using "/home/ulfha881/Desktop/Ulf//`v2'${Type}.dta",replace
	
	//running models and store results in "casctot_cat.dta"
	foreach var of varlist hg*{
		noi di "`var'"
		if "${Type}"=="full" cap xtologit `v2' `var' age carb protein fiber sbp dbp ///
		chol hdl ldl tg bmi i.sex i.country i.smoke i.diab i.diab_med i.pa i.chol_med ///
		i.bp_med i.site_plate*,or
		if "${Type}"=="basic" cap xtologit `v2' `var' age ///
		i.sex i.country i.site_plate*,or
		if _rc==0{
			matrix A=r(table)
