...
ppi <- bplapply(colnames(mgs), function(x) lm.fun(cacstot[which(data$ppi == "No")], mgs[which(data$ppi == "No"), x], cov[which(data$ppi == "No"), ]), BPPARAM = MulticoreParam(cores))
...
crohn <- bplapply(colnames(mgs), function(x) lm.fun(cacstot[which(data$crohn == "No")], mgs[which(data$crohn == "No"), x], cov[which(data$crohn == "No"), ]), BPPARAM = MulticoreParam(cores))
...
antib <- bplapply(colnames(mgs), function(x) lm.fun(cacstot[which(antib == "No")], mgs[which(antib == "No"), x], cov[which(antib == "No"), ]), BPPARAM = MulticoreParam(cores))
...
shannon <- bplapply(colnames(mgs), function(x) lm.fun(cacstot, mgs[, x], data.frame(cov, data$shannon)), BPPARAM = MulticoreParam(cores))
...
family <- data$family
family <- bplapply(colnames(mgs), function(x) lmer.fun(cacstot, mgs[, x], cov, family), BPPARAM = MulticoreParam(cores))
family <- do.call(rbind, family)
family$q.value <- p.adjust(family$p.value, method = "BH", n = sum(!is.na(family$p.value)))
family <- family[, c("n", "estimate", "se", "p.value", "q.value")]
colnames(family) <- paste0("family_", colnames(family))

# make and export table

res <- data.frame(name, id, level, ppi, crohn, antib, shannon, family)
