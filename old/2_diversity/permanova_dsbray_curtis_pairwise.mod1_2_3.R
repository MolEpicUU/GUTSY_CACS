...
#pairwise
dades.fun=dades[dades$casctot_cat==2| dades$casctot_cat==3,]
beta.fun<-beta[rownames(beta)%in%dades.fun$sample.id,names(beta)%in%dades.fun$sample.id]

print("identical")
print(identical(rownames(beta),dades.fun$sample.id))
...
res=data.frame(var1="2",var2="3",R2=fit$R2[1],p.value=fit$"Pr(>F)"[1])

write.table(res,file=paste(output.path,"permanova_adonis2_dsbray_curtis_casctot_pairwise.mod1_2_3.tsv",sep=""),row.names=F,col.names=T,sep="\t")
