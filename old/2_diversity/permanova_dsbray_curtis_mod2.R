...
print("adonis mod2")
set.seed(12345)
fit=adonis2(as.matrix(beta)~casctot_cat+gender+agev1+siteid+siteid:plate+q005a+smokestatus+q134+diab_treat+HBP_treat+HC_treat+log.fibrer+log.energi,data=dades,by="margin",parallel=ncores,permutations=permutations)
res=data.frame(R2=fit$R2[1],p.value=fit$"Pr(>F)"[1])

write.table(res,file=paste(output.path,"permanova_adonis2_dsbray_curtis_casctot_mod2.tsv",sep=""),row.names=F,col.names=T,sep="\t")
